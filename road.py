...
class Road:
    def __init__(self, world):
        # Calculate the number of control points based on the window height and safe space
        self.num_ctrl_points = (int)((world.win_height + SAFE_SPACE) / SPACING) + 2

        self.last_ctrl_point = 0  # Initialize the index of the last control point
        self.ctrl_points = []  # List to store control points
        self.centerPoints = []  # List to store center points of the road
        self.pointsLeft = []  # List to store left border points of the road
        self.pointsRight = []  # List to store right border points of the road

        # Initialize control points
        for i in range(self.num_ctrl_points):
            self.ctrl_points.append(vect2d())

        # Initialize left, right, and center points
        ...
        # Initialize the first two control points to be straight
        ...
        self.next_point = NUM_POINTS  # Initialize the next point index

        # Create road segments
        for i in range(self.num_ctrl_points - 2):
            self.createSegment(i + 1)

        self.last_ctrl_point = self.num_ctrl_points - 1  # Update the last control point index
        self.bottomPointIndex = 0  # Initialize the bottom point index

    def calcBorders(self, i):
        # Calculate the borders of the road segment at index i
    ...
    def createSegment(self, index):
        ...
        ny = np.array([p2.y, p1.y])  # Inverted because scipy wants increasing x's (in this case y's)
    ...
    def update(self, world):
        # Update the road by creating new segments if necessary
...
def getPoint(i, cap):
    # Get the wrapped index within the range of 0 to cap-1
    return (i + cap) % cap